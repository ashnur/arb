...
var add = integer.arr.add
var subtract = integer.arr.subtract
var multiply = integer.arr.multiply
var divide = integer.arr.divide
var compare_abs = integer.arr.compare_abs
var equal = integer.arr.equal
...
var get_random_array = function(){ return integer.int_to_arr(get_random_bigint()) }
var Integer = as_generator(get_random_array)
...
var clone = function(arr){ return arr.slice(0) }
var print = console_log
...
function topolynom(a){
  var sign = (a[1] & 1) ? '-' : '+' 
  return '(' + sign + a.slice(2).map(topoly).join(sign) + ')'
}
...
//while ( times-- > 0 ) {
//console.log('--------------------------------------')
//  bang(get_random_array(), get_random_array(), get_random_array())
//}
//bang([ 8, 1, 9, 7, 6, 8, 8, 7 ], [ 7, 1, 5, 3, 8, 3, 9 ])

function bang(a, b, c){
  if ( !(back_substitution_div(a, b, c) ) ) {
